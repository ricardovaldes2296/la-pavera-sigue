...
const DrinksPanel: React.FC<DrinksPanelProps> = ({ 
  ...
  const handleOrder = (drink: Drink) => {
    ...
    const recipe = `📝 *Receta:*%0A${drink.instructions}`;
    
    const message = `${header}%0A${subHeader}%0A%0A${recipe}%0A%0A${historyInfo}`;
      ...
      <header className="px-6 pt-8 pb-4 bg-zinc-950 sticky top-0 z-20 border-b border-white/5">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
             ...
             <button onClick={() => setShowHistory(true)} className="text-zinc-400 hover:text-gold-400 transition-colors relative">
                <History size={20} />
                {orderHistory.length > 0 && <span className="absolute -top-1 -right-1 bg-gold-600 text-zinc-950 text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{orderHistory.length}</span>}
             </button>
          </div>
        </div>
        <h1 className="font-serif text-4xl italic text-white mb-6">Bebidas</h1>
        <div className="flex border-b border-white/10">
          <button onClick={() => setActiveTab(DrinkType.COCKTAIL)} className={`flex-1 pb-3 text-sm tracking-wider uppercase transition-all relative ${activeTab === DrinkType.COCKTAIL ? 'text-gold-400 font-semibold' : 'text-zinc-500 hover:text-zinc-300'}`}>Cócteles{activeTab === DrinkType.COCKTAIL && <span className="absolute bottom-0 left-0 w-full h-[1px] bg-gold-400"></span>}</button>
          <button onClick={() => setActiveTab(DrinkType.MOCKTAIL)} className={`flex-1 pb-3 text-sm tracking-wider uppercase transition-all relative ${activeTab === DrinkType.MOCKTAIL ? 'text-gold-400 font-semibold' : 'text-zinc-500 hover:text-zinc-300'}`}>Mocktails{activeTab === DrinkType.MOCKTAIL && <span className="absolute bottom-0 left-0 w-full h-[1px] bg-gold-400"></span>}</button>
        </div>
      </header>
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {filteredDrinks.map((drink, index) => (
          <div key={index} onClick={() => handleOrder(drink)} className="group cursor-pointer">
            <div className="flex justify-between items-start mb-2">
              <div><h3 className="font-serif text-2xl text-white group-hover:text-gold-300 transition-colors italic">{drink.name}</h3></div>
              <span className="text-xl opacity-80 filter grayscale group-hover:grayscale-0 transition-all">{drink.emoji}</span>
            </div>
            <p className="text-zinc-400 text-sm font-light leading-relaxed mb-3">{drink.description}</p>
            <div className="flex flex-wrap gap-x-3 gap-y-1">{drink.ingredients.map((ing, i) => <span key={i} className="text-[10px] uppercase tracking-wider text-zinc-600 border border-zinc-800 px-2 py-0.5 rounded-full">{ing}</span>)}</div>
            <div className="w-full h-[1px] bg-white/5 mt-6 group-hover:bg-gold-500/20 transition-colors"></div>
          </div>
        ))}
        <div className="h-12"></div>
      </div>
      <OrderHistoryModal isOpen={showHistory} onClose={() => setShowHistory(false)} history={orderHistory} />
    </div>
  );
};
